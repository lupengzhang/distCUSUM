...
Details about this package can be found in the DESCRIPTION file. The main R function of the package is called "DCCP_multiple_changepoints", which can be applied for detecting multiple change points in high dimensional data using the distance-based CUSUM method presented in the paper. If there is only a single change point, the function "DCCP_single_changepoint" can be used. Both functions use the modified L1 norm by default. Users can specify another relevant distance function through the corresponding distance-function argument. The package also includes the S&P500 and MIT cellphone data sets analyzed in the paper. The original S&P500 data are also available in the R package "HDDchangepoint". Further details about the data sets can be found in the folder "data" in the package above.

Two examples:

Single change point: the data are simulated with n=100 and p=500, where there is a change in the mean of observations at location 60. The variables follow an AR(1) correlation structure with rho=0.5. Below is the code to generate the data and apply the R package, along with the output:
...
mu <- 0.3
s <- 0.75
Deltamu <- sample(c(rep(0,(1-s)p),rep(mu,sp)))
rho <- 0.5
...
Obs_before <- mvrnorm(0.6n,mu=rep(0,p),Sigma=cov_matrix)
Obs_after <- mvrnorm(0.4n,mu=Deltamu,Sigma=cov_matrix)
